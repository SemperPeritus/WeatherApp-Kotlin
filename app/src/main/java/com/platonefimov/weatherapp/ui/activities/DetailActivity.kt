...
import com.platonefimov.weatherapp.extensions.color
import com.platonefimov.weatherapp.extensions.textColor
...
class DetailActivity : AppCompatActivity(), ToolbarManager {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        async(UI) {
            val result = bg {
                RequestDayForecastCommand(intent.getLongExtra(ID, -1)).
                        execute()
            }
    ...
    private fun bindWeather(vararg views: Pair<Int, TextView>) = views.forEach {
        val degree = "\u00b0"
        it.second.text = "${it.first}$degree"
